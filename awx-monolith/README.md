...
You need to modify your host machine's hosts file (Mac/Linux: `/etc/hosts`; Windows: `%systemroot%\system32\drivers\etc\hosts`), adding the lines below:
    ...
    192.168.6.62  awx-monolith.local

(Where `awx-monolith.local` is the hostname you have configured in the `Vagrantfile`).

After that is configured, you could visit http://awx-monolith.local/ in a browser, and you'll see the AWX login page. Log in with the default credentials:
